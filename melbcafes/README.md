...
Exploring Melbourne Cafes


## Installation

After cloning/downloading the repository, simply install melbcafes as a package into your target virtual environment:

    $ pip install PATH_TO_melbcafes
...
changes to the source code take immediate effect on the installed package.
    ...
    $ pip install -e PATH_TO_melbcafes
...
This script will also apply settings found in the module `melbcafes.prod_settings` (or a custom Python file supplied
with the `--settings` flag) and which takes precedence over the same settings
found in `melbcafes.settings`.
...
`run-melbcafes-prod` script to take effect even if you
...
for example) with the `melbcafes.wsgi` entry point
(defined in `wsgi.py`) like so:

    $ gunicorn melbcafes.wsgi
